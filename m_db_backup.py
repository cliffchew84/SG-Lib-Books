...
def q_user_bks_bids(db, username: str):
    ...
    output = []
    for i in db["user_books"].find(
        {"UserName": username}, {"_id": 0, "user_book_id": 0}
    ):
        output.append(i)
    return output
...
def query_user_info(db, username: str):
    """Return user username and password from mongo DB"""
    return db["users"].find_one({"UserName": username}, {"_id": 0, "HashedPassword": 0})
...
def event_tracking(db, table, username: str, event_name: str):
    ...
    newvalues = {"$set": {"UserName": username, event_name: login_time}}
